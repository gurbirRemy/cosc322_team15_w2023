...
public class MinimaxSearch {
 ...
 // This class is used to store the result of alpha-beta pruning
    ...
    */
    public static MovesGenerator.Move findBestMoveUsingAlphaBeta(Graph graph, Square currentPlayer, int depth, long startTime) {
    	AlphaBetaResult bestAlphaBetaMove = null;
    	bestAlphaBetaMove = alphaBetaPruning(startTime, graph, depth, Float.NEGATIVE_INFINITY, Float.POSITIVE_INFINITY, currentPlayer, null);
    ...
    */
    private static AlphaBetaResult alphaBetaPruning(long startTime, Graph graph, int searchLevel, float alpha, float beta, Square currentPlayer, MovesGenerator.Move prevMove) {
     ...
     // Iterate over all possible moves for the current player and calculate their scores
        	for (Map.Entry<MovesGenerator.Move, Graph> action : MovesGenerator.possibleMoves(graph, currentPlayer).entrySet()) {
                ...
                float optimalScore = alphaBetaPruning(startTime, action.getValue(), searchLevel - 1, alpha, beta, Square.BLACK, action.getKey()).heuristic;
             // If the current score is better than the current optimal score, update the optimal score and move
                if(System.currentTimeMillis() - startTime > 28000) {break;}
